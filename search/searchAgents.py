        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            if not hitsWall:
                ...
                if nextPos in self.corners:
                    if nextPos not in cornerCheck:
                        cornerCheck = cornerCheck + (nextPos, )
                cost = self.costFn(nextPos)
                successors.append(((nextPos, cornerCheck), action, cost))
        self._expanded += 1
        return successors
...
def cornersHeuristic(state, problem):
    ...
    for corner in corners:
        if corner not in cornerCheck:
            missing.append(corner)
    currPos = pos
    outVal = 0
    while not len(missing) == 0:
        distQ = util.PriorityQueue()
        for corner in missing:
            pri = abs(currPos[0] - corner[0]) + abs(currPos[1] - corner[1])
            distQ.push(corner, pri)
        target = distQ.pop()
        outVal += abs(currPos[0] - target[0]) + abs(currPos[1] - target[1])
        currPos = target
        missing.remove(currPos)
    return outVal
...
def foodHeuristic(state, problem):
    ...
    for food in foodList:
        foodDist = abs(currPos[0] - food[0]) + abs(currPos[1] - food[1])
        if foodDist > outVal:
            outVal = foodDist
    return outVal
